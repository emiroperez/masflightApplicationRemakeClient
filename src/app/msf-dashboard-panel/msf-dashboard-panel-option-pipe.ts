...
export class MsfDashboardPanelOptionPipe implements PipeTransform
{
    transform(items: any[], currentChartType: any): any[]
    {
        ...
        return items.filter (item => (((item.metaData == 2 || item.metaData == 4) && currentChartType.flags & ChartFlags.MAP && !(currentChartType.flags & ChartFlags.MAPBOX))
            || !(currentChartType.flags & ChartFlags.MAP) || (item.tabType === 'map' && currentChartType.flags & ChartFlags.MAPBOX)));
    }
}
